...
const ProjectList = () => {
  ...
  useEffect(() => {
    const p = (projects || []).filter((p) => p.status === "active").filter((e) => e.type !== "admin" || e.name === "Holliday");
    setActiveProjects(p);
  }, [projects]);
  ...
  const handleSearch = (searchedValue) => {
    const p = (projects || [])
      ...
      .filter((e) => e.type !== "admin" || e.name === "Holliday")
      .filter((e) => e.name.toLowerCase().includes(searchedValue.toLowerCase()));
    setActiveProjects(p);
  };
